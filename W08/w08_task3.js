function main()
{
    ...
    for ( var i = 0; i < nfaces; i++ )
    {
        var id = faces[i];
        var m1=0;
        var m2=192/scalars[id[1]];
        var m3=255/scalars[id[2]];
        var S0 =Math.floor(0*(scalars[id[0]])) ;
        var S1 = Math.floor(m2*(scalars[ id[1]])) ;
        var S2 = Math.floor(m3*(scalars[ id[2]])) ;
       // var S3 = Math.floor(510*(scalars[ id[2]])) ;//255
